...
function Auth.generate_md5slat_pass(cpass)
	if cpass == nil or type(cpass) ~= "string" then
		filelog.sys_error(filename, " [BASIC_AUTH] Auth.generate_md5slat_pass invalid param")
		return nil
	end
	local cpass_slat = "gameplatform"..cpass.."20161114"

	return md5.sumhexa(cpass_slat) 
end
